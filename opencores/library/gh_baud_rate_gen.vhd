...
begin
	...
	rCE <= '1' when (unsigned(count) = 1) else
	       '0';
...
begin
	if (rst = '1') then
		...
		rLD <= irLD;
		if (unsigned(count) > unsigned('0' & (rate(15 downto 1)))) then -- fixed 04/10/06
			rCLK <= '1';
		else
			rCLK <= '0';
		end if;
...
begin
	...
	C_LD <= '1' when (unsigned(count) = 1) else
	        '1' when (rLD = '1') else
	        '0';
	
	C_CE <= '1' when (unsigned(rate) > 1) else
	        '0';
